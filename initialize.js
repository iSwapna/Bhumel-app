...
function deploy(wasm) {
	exe(
		`stellar contract deploy --wasm ${wasm} --alias ${filenameNoExtension(wasm)} -- --admin ${process.env.STELLAR_ACCOUNT}`
	);
}

function deployAll() {
	const contractsDir = `${dirname}/.stellar/contract-ids`;
	// Remove existing contract files
	removeFiles(`${contractsDir}/*.json`);
...
function importContract({ id, alias }) {
	...
	console.log(`Created import for ${alias}, ${id}`);
}
